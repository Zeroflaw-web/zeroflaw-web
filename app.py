...
async def _run_fix_background(scan_id: str, entry: dict):
    ...
    try:
        ...
        b2_store.put_text(f"reports/{scan_id}-fix-result.html", f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Fixes Applied</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#020617;color:#e2e8f0;line-height:1.6;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}}.card{{background:#0F172A;border:1px solid rgba(255,255,255,0.05);border-radius:16px;padding:32px;max-width:960px;width:100%}}h2{{color:#22d4ee;font-size:20px;margin-bottom:4px;display:flex;align-items:center;gap:8px}}.subtitle{{color:#64748b;font-size:13px;margin-bottom:20px}}table{{width:100%;border-collapse:collapse;margin-bottom:20px;table-layout:fixed}}th{{text-align:left;color:#64748b;font-size:11px;padding:4px 8px;border-bottom:1px solid rgba(255,255,255,0.1);text-transform:uppercase;letter-spacing:1px}}th:nth-child(1){{width:32px}}th:nth-child(2){{width:30%}}td{{overflow:hidden}}.btn{{display:inline-block;padding:12px 24px;background:#22d4ee;color:#020617;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600;margin-right:8px}}.btn-sec{{display:inline-block;padding:12px 24px;background:rgba(255,255,255,0.05);color:#e2e8f0;border:1px solid rgba(255,255,255,0.08);border-radius:10px;text-decoration:none;font-size:14px}}</style></head><body><div class="card"><h2>🛠 Fixes Applied ({len(changes)})</h2><p class="subtitle">The following changes were made to your project:</p><table><tr><th></th><th>Location</th><th>Issue</th><th>Fix</th></tr>{fix_rows}</table><div><a href="/download/{scan_id}?format=zip" class="btn" download>⬇ Download Fixed Source (.zip)</a><a href="/download/{scan_id}" class="btn-sec">← Back to Report</a></div></div></body></html>""")
        upd = b2_store.get(scan_id) or {}
        upd["status"] = "done_fix"
