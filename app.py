...
async def _run_fix_background(scan_id: str, entry: dict):
    ...
    try:
        ...
        if not changes:
            b2_store.put_text(f"reports/{scan_id}-fix-result.html", f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>No Fixable Issues</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#020617;color:#e2e8f0;line-height:1.6;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}}.card{{background:#0F172A;border:1px solid rgba(255,255,255,0.05);border-radius:16px;padding:32px;max-width:520px;width:100%;text-align:center}}h2{{color:#3fb950;font-size:20px;margin-bottom:12px}}p{{color:#94a3b8;font-size:13px;margin-bottom:16px;line-height:1.5}}a{{display:inline-block;padding:10px 24px;background:#22d4ee;color:#020617;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600}}</style></head><body><div class="card"><h2>✓ No Dependency Updates Needed</h2><p>All your dependencies are already at patched versions. Code-level issues (Bandit, Semgrep, secrets) require human review and are not auto-fixed — they should be triaged manually in your report.</p><a href="/download/{scan_id}">← Back to Report</a></div></body></html>""")
            upd = b2_store.get(scan_id) or {}
            upd["status"] = "done_fix"
            upd["fix_count"] = 0
            b2_store.put(scan_id, upd)
        ...
        fix_rows = ""
        for c in changes:
            fp = _short_path(str(c.get('file', '')))
            loc = f"{fp}:{c.get('line', '')}" if c.get('line') else fp
            issue = _short_text(str(c.get('issue', '')))
            fix = _short_text(str(c.get('fix', '')))
            fix_rows += f"""<tr><td style="padding:6px 8px;border-bottom:1px solid rgba(255,255,255,0.05);font-size:12px;color:#22d4ee;text-align:center;">🔧</td><td style="padding:6px 8px;border-bottom:1px solid rgba(255,255,255,0.05);font-size:12px;color:#94a3b8;word-break:break-all;">{loc}</td><td style="padding:6px 8px;border-bottom:1px solid rgba(255,255,255,0.05);font-size:12px;color:#94a3b8;word-break:break-word;">{issue}</td><td style="padding:6px 8px;border-bottom:1px solid rgba(255,255,255,0.05);font-size:12px;color:#3fb950;word-break:break-word;">{fix}</td></tr>"""

        b2_store.put_text(f"reports/{scan_id}-fix-result.html", f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Dependency Versions Updated</title><style>*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#020617;color:#e2e8f0;line-height:1.6;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}}.card{{background:#0F172A;border:1px solid rgba(255,255,255,0.05);border-radius:16px;padding:32px;max-width:960px;width:100%}}h2{{color:#22d4ee;font-size:20px;margin-bottom:4px;display:flex;align-items:center;gap:8px}}.subtitle{{color:#94a3b8;font-size:12px;margin-bottom:20px;max-width:700px}}table{{width:100%;border-collapse:collapse;margin-bottom:20px;table-layout:fixed}}th{{text-align:left;color:#64748b;font-size:11px;padding:4px 8px;border-bottom:1px solid rgba(255,255,255,0.1);text-transform:uppercase;letter-spacing:1px}}th:nth-child(1){{width:32px}}th:nth-child(2){{width:30%}}td{{overflow:hidden}}.muted{{color:#64748b;font-size:11px;margin-bottom:8px}}.muted a{{color:#22d4ee}}.btn{{display:inline-block;padding:12px 24px;background:#22d4ee;color:#020617;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600;margin-right:8px}}.btn-sec{{display:inline-block;padding:12px 24px;background:rgba(255,255,255,0.05);color:#e2e8f0;border:1px solid rgba(255,255,255,0.08);border-radius:10px;text-decoration:none;font-size:14px}}</style></head><body><div class="card"><h2>🛠 Dependency Versions Updated ({len(changes)})</h2><p class="subtitle">ZeroFlaw pinned vulnerable dependencies to their first patched version. Auto-fix does <strong>not</strong> modify your source code logic — only dependency version strings and lock files are changed. Review the diff before deploying.</p><table><tr><th></th><th>Location</th><th>Issue</th><th>Fix</th></tr>{fix_rows}</table><div class="muted">Code-level issues (bandit, semgrep, secrets) require human review and are not auto-fixed.</div><div style="margin-top:16px"><a href="/download/{scan_id}?format=zip" class="btn" download>⬇ Download Fixed Source (.zip)</a><a href="/download/{scan_id}" class="btn-sec">← Back to Report</a></div></div></body></html>""")
        upd = b2_store.get(scan_id) or {}
        upd["status"] = "done_fix"
